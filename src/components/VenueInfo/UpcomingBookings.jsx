...
export default function UpcomingBookings({ bookings, venueId, handleDelete, isDeleting }) {
    return (
        <div className="mb-10">
            <h3 className="text-xl font-normal mb-2.5">Bookings</h3>
            {bookings && bookings.length > 0 ? (
                <ul className="space-y-4">
                    {bookings.map((booking) => (
                        <li key={booking.id} className="border p-3 rounded-md bg-gray-100">
                            <p className="text-sm"><strong>Customer:</strong> {booking.customer.name}</p>
                            <p className="text-sm"><strong>Email:</strong> {booking.customer.email}</p>
